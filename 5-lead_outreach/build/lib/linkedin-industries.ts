/**
 * LinkedIn's industry taxonomy (v2), as the pinned actor's `industryIds`
 * filter expects it: numeric ids, sent as strings.
 *
 * GENERATED, do not hand-edit. Source, fetched 2026-09-24:
 *   https://raw.githubusercontent.com/HarvestAPI/linkedin-industry-codes-v2/
 *     main/linkedin_industry_code_v2_all_eng_with_header.csv
 * That is the table the actor's own input schema links to, which is why it is
 * used rather than a list assembled by hand.
 *
 * Labels are unique across all 434 entries, so a label is a safe key.
 * Matching and aliasing live in `industries.ts`, deliberately not here: this
 * file is data and carries no policy.
 */
export const LINKEDIN_INDUSTRIES: ReadonlyArray<readonly [id: string, label: string]> = [
